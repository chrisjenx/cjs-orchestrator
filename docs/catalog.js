...
const ARCHETYPE = {
  ...
  executor:   { tools: 'All tools', model: 'sonnet · build', note: 'The one agent that NESTS — spawns its own writers, test-writers, and evaluator loops for its phase.' },
  codegen:    { tools: 'All tools', model: 'sonnet · build (UI → opus)', note: 'Writes production code AND its tests together, following project templates.' },
  ...
  auditRO:    { tools: 'Bash, Glob, Grep, Read', model: 'opus · judgement (cheap refuters → haiku)', note: 'A flat parallel LEAF — never nests. One narrow lens over the diff; returns findings only.' },
  reviewer:   { tools: 'All tools', model: 'tier by depth (haiku → opus)', note: 'Checks changed files against one convention/quality dimension; fixes in place or flags.' },
...
const CATALOG = {
  /* ---- skills ---- */
  '/develop': { kind: 'skill', role: 'Spec → reviewed, committed branch — autonomously.',
  ...
  /* ---- planners ---- */
  'spec-analyzer': { kind: 'agent', arche: 'planner', role: 'Specs → validated requirements + file manifest + execution strategy.',
    detail: 'Dispatched when a spec exists. Reads the spec, validates it against the codebase, runs gap analysis, and emits plan sections the orchestrator uses directly — no follow-up planner needed.' },
  ...
  'ui-evaluator': { kind: 'agent', arche: 'evaluator', role: 'Scores generated UI against a rubric.',
    detail: 'The discriminator in the GAN loop: PASS with scores, or FAIL with actionable feedback. Deliberately a cheaper tier than the writer it judges — adversarial, not collusive.' },

  /* ---- auditors (parallel fan-out leaves) ---- */
  'audit-completeness-checker': { kind: 'agent', arche: 'auditRO', role: 'Cross-layer integration gaps: DB → repo → service → API → UI.',
  ...
  'Linear MCP': { kind: 'tool', build: 'provided', role: 'Fetch tickets, docs, and attachments.',
  ...
  'git: docs branch': { kind: 'tool', build: 'pattern', role: 'Durable, auth-free artifact store.',
    detail: 'An orphan git branch holding heavy design docs and mocks. Sub-agents fetch with `git show` — no external service to deauth or rate-limit them mid-run. A convention worth stealing.' },
...
Heavy detail lives in @.claude/references/*.md, not here.`,
    notes: [
      'Frontmatter is just <code>name</code> + <code>description</code>. The description is the trigger — write it for matching, not for humans.',
...
model: haiku          # cost tier — cheap mechanical … expensive judgement
effort: medium
...
## What to return
A short pointer. Your final message IS the result — return data,
...
service-to-service callers bypass them.
...
Long examples → @.claude/references/permission-patterns.md`,
    notes: [
      ...
      'Rules train the model\'s style as much as they instruct — write them in the shape you want code comments to take.',
    ],
  },
  {
  ...
  fanout: {
    ...
    body: `
      ...
      <div class="insight-row">
        ...
        <p>Dynamic Workflow lets the <em>model</em> choose the fan-out shape live — "spawn 16 finders × 3 verifiers to be safe." Nothing caps that instinct, so spend has no ceiling. The SubDriven skill fixes the shape at <em>authoring</em> time — phases, dispatch, the gate ladder, the self-scaling reviewer count, all tuned before the run. The model can't conjure five more verifiers mid-flight. <strong>Predictability is structural, not luck.</strong></p>
      </div>
      <div class="insight-row">
      ...
      <div class="insight-row">
        ...
        <p>Its fan-out is <em>heterogeneous and decided live</em> — many different prompts, each a unique cold prefix — and if the script stalls between waves, the 5-minute cache TTL lapses and the next wave pays cold again. The skill's fixed, phased dispatch keeps shared prefixes (CLAUDE.md, the standard agent briefs) hot and reused — better cache locality, tighter spend.</p>
      </div>
      <p class="insight-foot"><strong>Measured:</strong> effective tokens per run, representative figures from a real project's runs (anonymized). <strong>Reasoned:</strong> the cache mechanics above — directional, from architecture plus the documented 5-minute cache TTL and ~1.25× / ~0.1× write / read rates.</p>`,
  },
};
