# Scaffold eval — run `/develop:init` for real and validate what it writes

The detection eval ([README](./README.md)) is **read-only**: it checks Phase 1–2 discovery and
forbids writes. This is the **write** eval: it runs init's full Phase 0–4 in a sandbox, captures
...
`/develop:init` is LLM-driven, so its output is never bit-identical. Only the parts sourced
...
3. **Validate** each run against the stack's expected discovery:
   ```sh
   python3 scripts/check-scaffold.py /tmp/develop-init-eval/run1 evals/fixtures/<stack>/expected.json
   ```
4. **Variance** — the deterministic core must be identical across runs:
   ```sh
   python3 scripts/check-scaffold.py --compare /tmp/develop-init-eval/run1 /tmp/develop-init-eval/run2
   ```
5. **Cleanup** — `rm -rf /tmp/develop-init-eval`.

### Subagent contract (the run step)

Point it at the plugin (read-only) and the sandbox (write target):

- Plugin `plugins/develop`: execute `skills/init/SKILL.md`, reading the references it cites as
...
- Target = the sandbox dir: **every** file init writes (`.claude/...`, `CLAUDE.md`) goes there
...
To check init *recreates* a repo you already set up by hand, run **from inside that repo's own
checkout** so nothing private ever leaves it:

1. Sandbox a copy with `.claude/` and `CLAUDE.md` removed; run init into it (steps 1–2).
2. `check-scaffold.py validate <sandbox> <that repo's expected.json>` — does a fresh init produce
...
- **Should match:** gates, stack, config shape, model/cap defaults, the safe hook, the empty
  flywheel seed. A divergence here is an **init bug** — fix the skill.
- **Will differ, expected:** routing specialists, promoted `CLAUDE.md` anchors, anything in
  `.claude/agents/`. That is growth the flywheel added over many runs, not init's day-one job.

## Fixing failures (Iron Law)

A red check drives a fix in `skills/init/SKILL.md` or the references it reads — never a tweak to
